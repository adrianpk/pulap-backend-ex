defmodule PulapWeb.RolePermissionController do
  ...
  def create(conn, %{"role_permission" => role_permission_params}) do
    with {:ok, %RolePermission{} = role_permission} <- Auth.create_role_permission(role_permission_params) do
      conn
      |> put_status(:created)
      |> put_resp_header("location", role_permission_path(conn, :show, role_permission))
  ...
  def update(conn, %{"id" => id, "role_permission" => role_permission_params}) do
    ...
    with {:ok, %RolePermission{} = role_permission} <- Auth.update_role_permission(role_permission, role_permission_params) do
      render(conn, "show.json", role_permission: role_permission)
    end
  end
