defmodule PulapWeb.API.V1.PlanSubscriptionController do
  ...
  def create(conn, %{"plan_subscription" => plan_subscription_params}) do
    with {:ok, %PlanSubscription{} = plan_subscription} <-
           Biz.create_plan_subscription(plan_subscription_params) do
      conn
      |> put_status(:created)
      |> put_resp_header(
        "location",
        api_v1_plan_subscription_path(conn, :show, plan_subscription)
      )
  ...
  def update(conn, %{"id" => id, "plan_subscription" => plan_subscription_params}) do
    ...
    with {:ok, %PlanSubscription{} = plan_subscription} <-
           Biz.update_plan_subscription(plan_subscription, plan_subscription_params) do
      render(conn, "show.json", plan_subscription: plan_subscription)
    end
  end
