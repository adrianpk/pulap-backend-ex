defmodule PulapWeb.PermissionControllerTest do
  ...
  describe "update permission" do
    ...
    test "renders permission when data is valid", %{conn: conn, permission: %Permission{id: id} = permission} do
      conn = put conn, api_v1_permission_path(conn, :update, permission), permission: @update_attrs
    ...
    test "renders errors when data is invalid", %{conn: conn, permission: permission} do
      conn = put conn, api_v1_permission_path(conn, :update, permission), permission: @invalid_attrs
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
