defmodule PulapWeb.ManagershipControllerTest do
  ...
  describe "update managership" do
    ...
    test "renders managership when data is valid", %{
      conn: conn,
      managership: %Managership{id: id} = managership
    } do
      conn =
        put(conn, api_v1_managership_path(conn, :update, managership), managership: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, managership: managership} do
      conn =
        put(
          conn,
          api_v1_managership_path(conn, :update, managership),
          managership: @invalid_attrs
        )
