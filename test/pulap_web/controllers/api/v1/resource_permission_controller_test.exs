defmodule PulapWeb.ResourcePermissionControllerTest do
  ...
  describe "create resource_permission" do
    test "renders resource_permission when data is valid", %{conn: conn} do
      conn =
        post(
          conn,
          api_v1_resource_permission_path(conn, :create),
          resource_permission: @create_attrs
        )
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn =
        post(
          conn,
          api_v1_resource_permission_path(conn, :create),
          resource_permission: @invalid_attrs
        )
  ...
  describe "update resource_permission" do
    ...
    test "renders resource_permission when data is valid", %{
      conn: conn,
      resource_permission: %ResourcePermission{id: id} = resource_permission
    } do
      conn =
        put(
          conn,
          api_v1_resource_permission_path(conn, :update, resource_permission),
          resource_permission: @update_attrs
        )
  ...
  describe "delete resource_permission" do
    ...
    test "deletes chosen resource_permission", %{
      conn: conn,
      resource_permission: resource_permission
    } do
