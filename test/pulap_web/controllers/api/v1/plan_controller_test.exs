defmodule PulapWeb.PlanControllerTest do
  ...
  describe "index" do
    test "lists all plans", %{conn: conn} do
      conn = get(conn, api_v1_plan_path(conn, :index))
  ...
  describe "create plan" do
    test "renders plan when data is valid", %{conn: conn} do
      conn = post(conn, api_v1_plan_path(conn, :create), plan: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, api_v1_plan_path(conn, :show, id))
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, api_v1_plan_path(conn, :create), plan: @invalid_attrs)
  ...
  describe "update plan" do
    ...
    test "renders plan when data is valid", %{conn: conn, plan: %Plan{id: id} = plan} do
      conn = put(conn, api_v1_plan_path(conn, :update, plan), plan: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, plan: plan} do
      conn = put(conn, api_v1_plan_path(conn, :update, plan), plan: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete plan" do
    ...
    test "deletes chosen plan", %{conn: conn, plan: plan} do
      conn = delete(conn, api_v1_plan_path(conn, :delete, plan))
      assert response(conn, 204)

      assert_error_sent(404, fn ->
        get(conn, api_v1_plan_path(conn, :show, plan))
      end)
    end
  end
